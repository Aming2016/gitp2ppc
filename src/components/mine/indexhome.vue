<template>
	<div class="indexhome">
		...
		<div id="indexhead">
			...
			<ul class="indexheadbottom indexheadclass_1" style="margin-bottom:0;">
        <li>
          <div>账户总额 
             <el-tooltip class="item" effect="dark" content="账户金额+待收本金+待收收益" placement="top">
            <img src="../../imgs/mine/tishione.png" alt="">
            </el-tooltip>
            </div>
          <div>{{userInfo.total}}<span>元</span></div>
        </li>
        <li>
          <div>可用余额 
            <el-tooltip class="item" effect="dark" content="当前账户里的金额" placement="top">
            <img src="../../imgs/mine/tishione.png" alt="">
            </el-tooltip>  
          </div>
          <div>{{parseInt(userInfo.available_remain)+parseInt(userInfo.freeze_amount)}}<span>元</span></div>
        </li>
        <li>
          <div>待收本金
            <el-tooltip class="item" effect="dark" content="已投资的剩余待回收的本金" placement="top">
            <img src="../../imgs/mine/tishione.png" alt="">
            </el-tooltip> </div>
          <div>{{userInfo.shall_receive_capital}}<span>元</span></div>
        </li>
        <li>
          <div>累计收益
            <el-tooltip class="item" effect="dark" content="已投资的剩余待回收的收益" placement="top">
            <img src="../../imgs/mine/tishione.png" alt="">
            </el-tooltip> </div>
          <div>{{userInfo.accumulative_income}}<span>元</span></div>
        </li>
        <li>
          <div>累计投资
            <el-tooltip class="item" effect="dark" content="已投资的剩余待回收的收益" placement="top">
          ...
          <div>{{userInfo.investment_income_item_capital}}<span>元</span></div>
        </li>
        <li>
          <div>体验金
            <el-tooltip class="item" effect="dark" content="参加活动的用户的体验金" placement="top">
            <img src="../../imgs/mine/tishione.png" alt="">
            </el-tooltip> </div>
          <div>{{userInfo.experience_money?userInfo.experience_money.amount:'0'}}<span>元</span></div>
        </li>
			</ul>
      <!-- <ul class="indexheadbottom">
        <li>
          <div>可用余额 
            <el-tooltip class="item" effect="dark" content="账户金额-冻结资金" placement="top">
            <img src="../../imgs/mine/tishione.png" alt="">
            </el-tooltip> </div>
          <div>{{userInfo.available_remain}}<span>元</span></div>
        </li>
        <li>
          <div>冻结资金
            <el-tooltip class="item" effect="dark" content="当前账户冻结的金额" placement="top">
            <img src="../../imgs/mine/tishione.png" alt="">
            </el-tooltip> </div>
          <div>{{userInfo.freeze_amount}}<span>元</span></div>
        </li>
        <li>
          <div>累计投资
            <el-tooltip class="item" effect="dark" content="账户累计的投资金额" placement="top">
            <img src="../../imgs/mine/tishione.png" alt="">
            </el-tooltip> </div>
          <div>{{userInfo.investment_income_item_capital}}<span>元</span></div>
        </li>
        <li>
          <div>累计收益
            <el-tooltip class="item" effect="dark" content="账户累计的投资收益" placement="top">
            <img src="../../imgs/mine/tishione.png" alt="">
            </el-tooltip> </div>
          <div>{{userInfo.accumulative_income}}<span>元</span></div>
        </li>
		...
		<div id="hk_jhindex">
			...
			<router-link id="hk_jboxul" @click.native="dataonebtn(item,index)" tag="li" :to="{path:'/borrowmoneydetails/', query: {id:item.id+' 我的投资'}}" v-for="item in borderset">
				<li class="hk_jboxli">					
					...
					<div>{{item.get_current_item.capital}}元</div>
		...
		<div id="hk_jhindex">
           ...
           <div class="biaoqianid" v-show="item.label_str!=''">
                <span>{{item.label_str}}</span>
           </div>
					<div id="hk_text1">
...
export default {
  ...
  methods: {
    ...
    _getUserDetail() {
      //获取用户信息
      this.$http.get(this.$url.URL + this.$url.MY_INDEX_DATA).then(response => {
    ...
    _getRepayDetails() {
      //还款计划(我的借款)
      try {
        this.$http.get(this.$url.URL + this.$url.MY_LOAN).then(response => {
...
.biaoqianid::before {
  ...
  height: 0;
  border-width: 40px;
  border-color: #ffc600 #ffc600 transparent transparent;
  border-style: solid;
}
#headtopx {
  width: 100px;
...
.indexheadclass_1 > li:nth-last-of-type(1) {
  border-left: 1px solid #e3e3e3;
}
.indexheadbottom > li > div > img {
  cursor: pointer;
}
.indexheadbottom {
  width: 100%;
