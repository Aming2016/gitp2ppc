...
export default {
  ...
  created() {
    ...
    this.resul =
      this.$url.QIANDUODUO +
      JSON.parse(window.sessionStorage.dataall).WITHDRAW_CALLBACK; //提现接口
  ...
  methods: {
    ...
    _balanceinquiry() {
      this.$http
        .post(
          this.$url.URL + this.$url.QUERY_BALANCE,
          {},
        ...
        if (this.my_data.deposit_auth) {
          ...
          if (this.money !== "" && reg.test(this.money)) {
            if (Number(this.money) <= Number(this.my_data.available_remain)) {
              this.$http
                .post(this.$url.URL + this.$url.MONEY_IN_OUT, {
                  //充值提现获取订单号
                  type: "RECHARGE",
                  amount: this.money
                })
                .then(response => {
                  ...
                  var str =
                    this.my_data.deposit_id +
                    this.p +
                    this.order_no +
                    this.money +
                    "0" +
                    this.my_data.bank_card_no +
                    this.my_data.is_credit_card +
                    this.my_data.bank_code +
                    this.my_data.province_code +
                    this.my_data.city_code +
                    this.returnurl +
                    this.resul;
                  this.$http
