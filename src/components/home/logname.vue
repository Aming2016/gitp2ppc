	...
	<div class="logname">
		<div v-if="moveone">
			...
			<div class="logbox">
				...
				<div class="logboxright">
					<input type="text" placeholder="请输入好友邀请码" v-model="codename"/>
...
export default {
  ...
  methods: {
      ...
      if (this.phonenumber !== "") {
        if (this.passwords !== "" && this.passworda !== "") {
          ...
          if (this.dxyzm !== "") {
            ...
            this.$http
              .post(this.$url.URL + this.$url.REGISTER, {
                //注册
                mobile: this.phonenumber,
		password: passwordsz,
		recommend_code:this.codename,
    ...
    movephoneer() {
      ...
      if (!/^1[23456789]\d{9}$/.test(this.phonenumber)) {
    ...
    password() {
      //验证密码
      if (
        !/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,16}$/.test(this.passwords)
      ) {
