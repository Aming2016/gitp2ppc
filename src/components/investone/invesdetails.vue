			...
			<div id="contentleft">
				<div id="tabmsg">
					<div>
						...
						<div style="margin-left:14px;line-height:50px;">{{dataid.name}}</div>
				...
				<div id="contentbox">
					...
					<ul id="contentultwo">
						<li><span style="vertical-align: top;">{{dataid.display_rate}}%</span><span v-if="dataid.label_str!==''" style="font-size:16px;color:red;vertical-align: top;">{{dataid.label_str}}加息</span></li>
			...
			<div id="contentright">
          	...
          	<span>
				...
				<div id="contentinputtext">
					<input type="text" v-model="money" :placeholder="'投资最大额度'+dataid.remain_amount" :disabled ="dataid.project_type=='4'"/>
...
export default {
  ...
  created() {
    ...
    if (window.sessionStorage.particulars == "moveurl") {
      this.$http
        .get(this.$url.URL + this.$url.PROJECT_MAIN + "/" + this.id + "/")
        .then(response => {
          ...
          document.title = this.dataid.name + "-全民金服借款标详情";
          ...
          for (let i = 0; i < this.dataid.project_summary.length; i++) {
            ...
            for (
              let l = 0;
              l < this.dataid.project_summary[i].project_detail_image.length;
              l++
            ) {
              this.dataImg.push(
      ...
      this.$http
        .get(this.$url.URL + this.$url.PROJECT_DETAIL + "/" + this.id + "/")
        .then(response => {
          if (response.data.issue_type == "MONTH") {
            response.data.issue_count = response.data.issue_count + "个月";
          } else {
  ...
  methods: {
    ...
    _myindexdata() {
      ...
      if (window.sessionStorage.myindexdata == undefined) {
        this.my_data = "";
        if (this.dataid.project_type == "4") {
          //如果是体验标project_type=4
          this.money = "体验标请先登录";
        }
      } else {
        ...
        if (this.dataid.project_type == "4") {
          //如果是体验标project_type=4
          this.money = this.my_data.experience_money
            ? this.my_data.experience_money.amount
            : "请先获取体验金";
        }
      }
    },
    investbtn() {
      if (this.dataid.project_type == "4") {
        this.investbtnone(); //是体验金支付的提交参数
      } else {
        this.investbtntwo(); //不是体验金支付的提交参数
      }
    },
    investbtnone() {
      //是体验金支付的提交参数
      if (window.sessionStorage.token == undefined) {
        ...
        //判断体验金否大于可投金额
        console.log(this.my_data.experience_money.amount);
        if (parseFloat(this.my_data.experience_money.amount) != 0) {
          if (
            parseFloat(this.my_data.experience_money.amount) <=
            parseFloat(this.dataid.remain_amount)
          ) {
            //判断是否阅读服务协议
            if (this.checked) {
              if (this.my_data.deposit_auth) {
                this.$confirm("是否进行投资?", "提示", {
                  confirmButtonText: "确定",
                  cancelButtonText: "取消",
                  type: "warning"
                })
                  .then(() => {
                    this._querystyjtz(); //调取体验金支付接口
                  })
                  .catch(() => {});
              } else {
                this.$alert("请先前往乾多多开户");
              }
            } else {
              this.$alert("请先阅读出借人服务协议");
            }
        } else {
          this.$alert("投资金额大于可投资金额");
        }
        } else {
            this.$alert("体验金已使用完");
          }
      ...
      this.$http
        .post(
          this.$url.URL + this.$url.PAYINVEST,
          {
            //投资的转账列表
            project_id: this.id,
            amount: this.my_data.experience_money.amount,
            is_pre: "0"
          }
        )
        .then(response => {
    ...
    _querysmydata() {
      this.$http
        ...
        .then(response => {
          ...
          console.log(JSON.parse(window.sessionStorage.myindexdata));
        });
    },
    investbtntwo() {
      //不是体验金支付的提交参数
            ...
            .then(({ value }) => {
              this.$http
                ...
                .then(response => {
                  if (response.data.code == 0) {
                    if (
                      Number(this.money) <= Number(this.dataid.remain_amount)
                    ) {
                      if (
                        Number(this.money) <=
                        Number(this.my_data.available_remain)
                      ) {
                        if (this.money % this.dataid.min_amount == "0") {
                          if (this.checked) {
                            this.$http
                              .post(
                                this.$url.URL + this.$url.PAYINVEST,
                                ...
                                if (response.data.LoanJsonList !== undefined) {
                                  for (let i = 0; i < list.length; i++) {
                                    if (list[i].SecondaryJsonList.length > 0) {
                                      list[
                                        i
                                      ].SecondaryJsonList = JSON.stringify(
          ...
          if (Number(this.money) <= this.dataid.remain_amount) {
            if (Number(this.money) <= Number(this.my_data.available_remain)) {
              if (this.money % this.dataid.min_amount == "0") {
                ...
                this.$alert(
                  "最低投资金额为" + this.dataid.min_amount + "的整数倍"
                );
              }
            } else {
              this.$alert("超出账户总余额请前往充值");
